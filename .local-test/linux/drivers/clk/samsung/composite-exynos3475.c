...
/* 4.14 has no vendor clock framework; this is a no-op compatibility flag. */
#ifndef CLK_IS_ROOT
#define CLK_IS_ROOT 0
#endif
#include "composite-exynos3475.h"
...
#define to_comp_pll(_hw) container_of(_hw, struct exynos3475_composite_pll, hw)
#define to_comp_mux(_hw) container_of(_hw, struct exynos3475_composite_mux, hw)
#define to_comp_divider(_hw) container_of(_hw, struct exynos3475_composite_divider, hw)
#define to_usermux(_hw) container_of(_hw, struct clk_exynos3475_usermux, hw)
...
static struct exynos3475_clk_reg_dump *reg_dump;
static unsigned long nr_reg_dump;

static int exynos3475_clk_suspend(void)
{
	struct exynos3475_clk_reg_dump *rd = reg_dump;
	unsigned long i;
...
static void exynos3475_clk_resume(void)
{
	struct exynos3475_clk_reg_dump *rd = reg_dump;
	unsigned long i;
...
static struct syscore_ops exynos3475_clk_syscore_ops = {
	.suspend	= exynos3475_clk_suspend,
	.resume		= exynos3475_clk_resume,
};
#endif /* CONFIG_PM_SLEEP */

/* setup the essentials required to support clock lookup using ccf */
void __init exynos3475_clk_context_init(struct device_node *np, void __iomem *base,
		...
		unsigned long nr_soc_rdump)
{
	...
	if (rdump && nr_rdump) {
		unsigned int idx;
		reg_dump = kzalloc(sizeof(struct exynos3475_clk_reg_dump)
		...
		register_syscore_ops(&exynos3475_clk_syscore_ops);
	}
#endif
}

/* add a clock instance to the clock lookup table used for dt based lookup */
static void exynos3475_clk_add_lookup(struct clk *clk, unsigned int id)
{
	if (clk_table && id)
		clk_table[id] = clk;
}
...
void __init exynos3475_register_fixed_rate(
		struct exynos3475_fixed_rate *list, unsigned int nr_clk)
{
	struct clk *clk;
	...
	for (idx = 0; idx < nr_clk; idx++, list++) {
		...
		exynos3475_clk_add_lookup(clk, list->id);
...
void __init exynos3475_register_fixed_factor(
		struct exynos3475_fixed_factor *list, unsigned int nr_clk)
{
	struct clk *clk;
	unsigned int idx, ret;
 ...
 */
void __init exynos3475_register_of_fixed_ext(
			struct exynos3475_fixed_rate *fixed_rate_clk,
			...
			struct of_device_id *clk_matches)
{
	...
	exynos3475_register_fixed_rate(fixed_rate_clk, nr_fixed_rate_clk);
}

/* operation functions for pll clocks */
static const struct exynos3475_pll_rate_table *exynos3475_get_pll_settings(
				struct exynos3475_composite_pll *pll, unsigned long rate)
{
	const struct exynos3475_pll_rate_table  *rate_table = pll->rate_table;
	int i;
...
static long exynos3475_pll_round_rate(struct clk_hw *hw,
			unsigned long drate, unsigned long *prate)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	const struct exynos3475_pll_rate_table *rate_table = pll->rate_table;
	int i;
...
static int exynos3475_composite_pll_is_enabled(struct clk_hw *hw)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
...
static int exynos3475_composite_pll_enable(struct clk_hw *hw)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	int set = pll->pll_flag & PLL_BYPASS ? 0 : 1;
	unsigned int reg;
...
static void exynos3475_composite_pll_disable(struct clk_hw *hw)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	int set = pll->pll_flag & PLL_BYPASS ? 0 : 1;
	unsigned int reg;
...
static unsigned long exynos3475_pll145xx_recalc_rate(struct clk_hw *hw,
...
static inline bool exynos3475_pll145xx_mp_check(u32 mdiv, u32 pdiv, u32 pll_con)
{
	return ((mdiv != ((pll_con >> PLL145XX_MDIV_SHIFT) & PLL145XX_MDIV_MASK)) ||
		(pdiv != ((pll_con >> PLL145XX_PDIV_SHIFT) & PLL145XX_PDIV_MASK)));
}

static int exynos3475_pll145xx_set_rate(struct clk_hw *hw, unsigned long drate,
					unsigned long prate)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	const struct exynos3475_pll_rate_table *rate;
	u32 pll_con;

	rate = exynos3475_get_pll_settings(pll, drate);
	if (!rate) {
	...
	pll_con = readl(pll->con_reg);
	if (!(exynos3475_pll145xx_mp_check(rate->mdiv, rate->pdiv, pll_con))) {
...
static unsigned long exynos3475_pll1460x_recalc_rate(struct clk_hw *hw,
				unsigned long parent_rate)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
...
static inline bool exynos3475_pll1460x_mpk_check(u32 mdiv, u32 pdiv, u32 kdiv, u32 pll_con0, u32 pll_con1)
{
...
static int exynos3475_pll1460x_set_rate(struct clk_hw *hw, unsigned long drate,
					unsigned long parent_rate)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	...
	pll_con0 = readl(pll->con_reg);
	pll_con1 = readl(pll->con_reg + 4);
	if (!(exynos3475_pll1460x_mpk_check(rate->mdiv, rate->pdiv, rate->kdiv, pll_con0, pll_con1))) {
...
static const struct clk_ops exynos3475_pll145xx_clk_ops = {
	.recalc_rate = exynos3475_pll145xx_recalc_rate,
	.set_rate = exynos3475_pll145xx_set_rate,
	.round_rate = exynos3475_pll_round_rate,
	.enable = exynos3475_composite_pll_enable,
	.disable = exynos3475_composite_pll_disable,
	.is_enabled = exynos3475_composite_pll_is_enabled,
};

static const struct clk_ops exynos3475_pll1460x_clk_ops = {
	.recalc_rate = exynos3475_pll1460x_recalc_rate,
	.set_rate = exynos3475_pll1460x_set_rate,
	.round_rate = exynos3475_pll_round_rate,
	.enable = exynos3475_composite_pll_enable,
	.disable = exynos3475_composite_pll_disable,
	.is_enabled = exynos3475_composite_pll_is_enabled,
...
static int exynos3475_composite_pll_enable_onchange(struct clk_hw *hw)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	int set = pll->pll_flag & PLL_BYPASS ? 0 : 1;
	unsigned int reg;
...
static void exynos3475_composite_pll_disable_onchange(struct clk_hw *hw)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	int set = pll->pll_flag & PLL_BYPASS ? 0 : 1;
	unsigned int reg;
...
static unsigned long exynos3475_pll255xx_recalc_rate(struct clk_hw *hw,
				unsigned long parent_rate)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	u32 mdiv, pdiv, sdiv, pll_con;
...
static inline bool exynos3475_pll255xx_mp_check(u32 mdiv, u32 pdiv, u32 pll_con)
{
	return ((mdiv != ((pll_con >> PLL255XX_MDIV_SHIFT) & PLL255XX_MDIV_MASK)) ||
		(pdiv != ((pll_con >> PLL255XX_PDIV_SHIFT) & PLL255XX_PDIV_MASK)));
}

static int exynos3475_pll255xx_set_rate(struct clk_hw *hw, unsigned long drate,
					unsigned long prate)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	const struct exynos3475_pll_rate_table *rate;
	...
	pll_con = readl(pll->con_reg);
	if (!(exynos3475_pll255xx_mp_check(rate->mdiv, rate->pdiv, pll_con))) {
...
static const struct clk_ops exynos3475_pll255xx_clk_ops = {
	.recalc_rate = exynos3475_pll255xx_recalc_rate,
	.set_rate = exynos3475_pll255xx_set_rate,
	.round_rate = exynos3475_pll_round_rate,
	.enable = exynos3475_composite_pll_enable_onchange,
	.disable = exynos3475_composite_pll_disable_onchange,
	.is_enabled = exynos3475_composite_pll_is_enabled,
};

static unsigned long exynos3475_pll2650x_recalc_rate(struct clk_hw *hw,
				unsigned long parent_rate)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	u32 mdiv, pdiv, sdiv, pll_con0, pll_con1;
...
static inline bool exynos3475_pll2650x_mpk_check(u32 mdiv, u32 pdiv, u32 kdiv, u32 pll_con0, u32 pll_con1)
{
...
static int exynos3475_pll2650x_set_rate(struct clk_hw *hw, unsigned long drate,
					unsigned long parent_rate)
{
	struct exynos3475_composite_pll *pll = to_comp_pll(hw);
	u32 pll_con0, pll_con1;
	...
	pll_con0 = readl(pll->con_reg);
	pll_con1 = readl(pll->con_reg + 4);
	if (!(exynos3475_pll2650x_mpk_check(rate->mdiv, rate->pdiv, rate->kdiv, pll_con0, pll_con1))) {
...
static const struct clk_ops exynos3475_pll2650x_clk_ops = {
	.recalc_rate = exynos3475_pll2650x_recalc_rate,
	.set_rate = exynos3475_pll2650x_set_rate,
	.round_rate = exynos3475_pll_round_rate,
	.enable = exynos3475_composite_pll_enable_onchange,
	.disable = exynos3475_composite_pll_disable_onchange,
	.is_enabled = exynos3475_composite_pll_is_enabled,
...
/* register function for pll clocks */
static void _exynos3475_register_comp_pll(struct exynos3475_composite_pll *list)
{
	struct clk *clk;
	...
	if (list->type == pll_1450x)
		clk = clk_register_composite(NULL, list->name, pname, 1,
				NULL, NULL,
				&list->hw, &exynos3475_pll145xx_clk_ops,
				&list->hw, &exynos3475_pll145xx_clk_ops, list->flag);
		...
		clk = clk_register_composite(NULL, list->name, pname, 1,
				NULL, NULL,
				&list->hw, &exynos3475_pll1460x_clk_ops,
				&list->hw, &exynos3475_pll1460x_clk_ops, list->flag);
	else if (list->type == pll_2551x || list->type == pll_2555x)
		clk = clk_register_composite(NULL, list->name, pname, 1,
				NULL, NULL,
				&list->hw, &exynos3475_pll255xx_clk_ops,
				&list->hw, &exynos3475_pll255xx_clk_ops, list->flag);
	else if (list->type == pll_2650x)
		clk = clk_register_composite(NULL, list->name, pname, 1,
				NULL, NULL,
				&list->hw, &exynos3475_pll2650x_clk_ops,
				&list->hw, &exynos3475_pll2650x_clk_ops, list->flag);
	else {
...
void exynos3475_register_comp_pll(struct exynos3475_composite_pll *list,
				unsigned int nr_pll)
{
	int cnt;

	for (cnt = 0; cnt < nr_pll; cnt++)
		_exynos3475_register_comp_pll(&list[cnt]);
}

/* operation functions for mux clocks */
static u8 exynos3475_mux_get_parent(struct clk_hw *hw)
{
	struct exynos3475_composite_mux *mux = to_comp_mux(hw);
	u32 val;
...
static int exynos3475_mux_set_parent(struct clk_hw *hw, u8 index)
{
	struct exynos3475_composite_mux *mux = to_comp_mux(hw);
	u32 val;
...
static const struct clk_ops exynos3475_composite_mux_ops = {
	.get_parent = exynos3475_mux_get_parent,
	.set_parent = exynos3475_mux_set_parent,
};

/* operation functions for mux clocks checking status with "on changing" */

static int exynos3475_mux_set_parent_onchange(struct clk_hw *hw, u8 index)
{
	struct exynos3475_composite_mux *mux = to_comp_mux(hw);
	u32 val;
	unsigned long flags = 0;
...
static const struct clk_ops exynos3475_composite_mux_ops_onchange = {
	.get_parent = exynos3475_mux_get_parent,
	.set_parent = exynos3475_mux_set_parent_onchange,
};

/* register function for mux clock */
static void _exynos3475_register_comp_mux(struct exynos3475_composite_mux *list)
{
	struct clk *clk;
	unsigned int ret = 0;
	...
	if (!(list->flag & CLK_ON_CHANGING))
		clk = clk_register_composite(NULL, list->name, list->parents, list->num_parents,
				&list->hw, &exynos3475_composite_mux_ops,
				NULL, NULL,
				NULL, NULL, list->flag);
	else
		clk = clk_register_composite(NULL, list->name, list->parents, list->num_parents,
				&list->hw, &exynos3475_composite_mux_ops_onchange,
				NULL, NULL,
				NULL, NULL, list->flag);
...
void exynos3475_register_comp_mux(struct exynos3475_composite_mux *list,
				unsigned int nr_mux)
{
	int cnt;

	for (cnt = 0; cnt < nr_mux; cnt++)
		_exynos3475_register_comp_mux(&list[cnt]);
}

/* operation functions for divider clocks */
static unsigned long exynos3475_divider_recalc_rate(struct clk_hw *hw,
		unsigned long parent_rate)
{
	struct exynos3475_composite_divider *divider = to_comp_divider(hw);
	unsigned int val;
...
static int exynos3475_divider_bestdiv(struct clk_hw *hw, unsigned long rate,
...
static long exynos3475_divider_round_rate(struct clk_hw *hw, unsigned long rate,
				unsigned long *prate)
{
	int div = 1;

	div = exynos3475_divider_bestdiv(hw, rate, prate);
	if (div == 0) {
...
static int exynos3475_divider_set_rate(struct clk_hw *hw, unsigned long rate,
				unsigned long parent_rate)
{
	struct exynos3475_composite_divider *divider = to_comp_divider(hw);
	unsigned int div;
...
static const struct clk_ops exynos3475_composite_divider_ops = {
	.recalc_rate = exynos3475_divider_recalc_rate,
	.round_rate = exynos3475_divider_round_rate,
	.set_rate = exynos3475_divider_set_rate,
};

/* register function for divider clocks */
static void _exynos3475_register_comp_divider(struct exynos3475_composite_divider *list)
{
	struct clk *clk;
	unsigned int ret = 0;
	...
	clk = clk_register_composite(NULL, list->name, &list->parent_name, 1,
			NULL, NULL,
			&list->hw, &exynos3475_composite_divider_ops,
			NULL, NULL, list->flag);
...
void  exynos3475_register_comp_divider(struct exynos3475_composite_divider *list,
				unsigned int nr_div)
{
	int cnt;

	for (cnt = 0; cnt < nr_div; cnt++)
		_exynos3475_register_comp_divider(&list[cnt]);
}
...
int exynos3475_add_clk_gate_list(struct clk *clk, unsigned long offset, u8 bit_idx, const char *name)
{
...
struct clk *exynos3475_clk_get_by_reg(unsigned long offset, u8 bit_idx)
{
	unsigned int i;
...
static struct clk * __init _exynos3475_register_gate(struct exynos3475_gate *list)
{
	struct clk *clk;
	unsigned int ret = 0;
...
void __init exynos3475_register_gate(struct exynos3475_gate *list,
			unsigned int nr_gate)
{
	...
	for (cnt = 0; cnt < nr_gate; cnt++) {
		clk = _exynos3475_register_gate(&list[cnt]);
		...
		/* Make list for gate clk to used by exynos3475_clk_get_by_reg */
		if (!gate_list_fail)
			exynos3475_add_clk_gate_list(clk, (unsigned long)(list[cnt].reg), list[cnt].bit, list[cnt].name);
	}
...
static int exynos3475_usermux_is_enabled(struct clk_hw *hw)
{
	struct clk_exynos3475_usermux *usermux = to_usermux(hw);
	u32 val;
...
static int exynos3475_usermux_enable(struct clk_hw *hw)
{
	struct clk_exynos3475_usermux *usermux = to_usermux(hw);
	u32 val;
	unsigned long flags = 0;
...
static void exynos3475_usermux_disable(struct clk_hw *hw)
{
	struct clk_exynos3475_usermux *usermux = to_usermux(hw);
	u32 val;
	unsigned long flags = 0;
...
static const struct clk_ops exynos3475_usermux_ops = {
	.enable = exynos3475_usermux_enable,
	.disable = exynos3475_usermux_disable,
	.is_enabled = exynos3475_usermux_is_enabled,
};

/* register function for usermux clocks */
static struct clk * __init _exynos3475_register_comp_usermux(struct exynos3475_usermux *list)
{
	struct clk_exynos3475_usermux *usermux;
	struct clk *clk;
	struct clk_init_data init;

	usermux = kzalloc(sizeof(struct clk_exynos3475_usermux), GFP_KERNEL);
	if (!usermux) {
	...
	init.name = list->name;
	init.ops = &exynos3475_usermux_ops;
...
void __init exynos3475_register_usermux(struct exynos3475_usermux *list,
			unsigned int nr_usermux)
{
	...
	for (cnt = 0; cnt < nr_usermux; cnt++) {
		clk = _exynos3475_register_comp_usermux(&list[cnt]);
		if (IS_ERR(clk)) {
			pr_err("%s: failed to register clock %s\n", __func__,
					(&list[cnt])->name);
			return;
		...
		exynos3475_clk_add_lookup(clk, (&list[cnt])->id);
